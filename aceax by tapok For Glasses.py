...
class GlassesConfigGenerator:
    def __init__(self, master):
        self.master = master
        self.master.title("Arma 3 Glasses Config Generator")
        ...
        self.glasses_frame = ttk.Frame(self.notebook)

        self.notebook.add(self.cfg_patches_frame, text="CfgPatches")
        self.notebook.add(self.glasses_frame, text="Glasses")

        self.create_cfg_patches_widgets()
        self.create_glasses_widgets()
    ...
    def create_glasses_widgets(self):
        self.glasses_canvas = tk.Canvas(self.glasses_frame)
        self.glasses_scrollbar = ttk.Scrollbar(self.glasses_frame, orient="vertical", command=self.glasses_canvas.yview)
        self.glasses_scrollable_frame = ttk.Frame(self.glasses_canvas)

        self.glasses_scrollable_frame.bind(
            "<Configure>",
            lambda e: self.glasses_canvas.configure(
                scrollregion=self.glasses_canvas.bbox("all")
            )
        )

        self.glasses_canvas.create_window((0, 0), window=self.glasses_scrollable_frame, anchor="nw")
        self.glasses_canvas.configure(yscrollcommand=self.glasses_scrollbar.set)

        self.glasses_canvas.pack(side="left", fill="both", expand=True)
        self.glasses_scrollbar.pack(side="right", fill="y")

        self.create_cfg_glasses_widgets()
        self.create_aceax_widgets()

    def create_cfg_glasses_widgets(self):
        ttk.Label(self.glasses_scrollable_frame, text="CfgGlasses").pack(anchor='w', padx=10, pady=5)
        
        ttk.Label(self.glasses_scrollable_frame, text="Glasses Base Class:").pack(anchor='w', padx=10, pady=5)
        self.glasses_base_class = ttk.Entry(self.glasses_scrollable_frame)
        self.glasses_base_class.pack(fill=tk.X, padx=10, pady=5)
        self.glasses_base_class.insert(0, "None")

        ttk.Label(self.glasses_scrollable_frame, text="Glasses Mother Class:").pack(anchor='w', padx=10, pady=5)
        self.glasses_mother_class = ttk.Entry(self.glasses_scrollable_frame)
        self.glasses_mother_class.pack(fill=tk.X, padx=10, pady=5)
        self.glasses_mother_class.insert(0, "H_HeadBandage_base_F_t")

        ttk.Label(self.glasses_scrollable_frame, text="Model:").pack(anchor='w', padx=10, pady=5)
        self.model = ttk.Entry(self.glasses_scrollable_frame)
        self.model.pack(fill=tk.X, padx=10, pady=5)
        self.model.insert(0, ".p3d")

        ttk.Label(self.glasses_scrollable_frame, text="Picture:").pack(anchor='w', padx=10, pady=5)
        self.picture = ttk.Entry(self.glasses_scrollable_frame)
        self.picture.pack(fill=tk.X, padx=10, pady=5)
        self.picture.insert(0, ".paa")

        ttk.Label(self.glasses_scrollable_frame, text="Icon:").pack(anchor='w', padx=10, pady=5)
        self.icon = ttk.Entry(self.glasses_scrollable_frame)
        self.icon.pack(fill=tk.X, padx=10, pady=5)
        self.icon.insert(0, ".paa")

        self.hidden_selections_frame = ttk.Frame(self.glasses_scrollable_frame)
        ...
        self.hidden_textures_frame = ttk.Frame(self.glasses_scrollable_frame)
    ...
    def create_aceax_widgets(self):
        ttk.Label(self.glasses_scrollable_frame, text="AceAx").pack(anchor='w', padx=10, pady=5)

        ttk.Label(self.glasses_scrollable_frame, text="AceAx Model:").pack(anchor='w', padx=10, pady=5)
        self.aceax_model = ttk.Entry(self.glasses_scrollable_frame)
        self.aceax_model.pack(fill=tk.X, padx=10, pady=5)

        ttk.Label(self.glasses_scrollable_frame, text="AceAx Description:").pack(anchor='w', padx=10, pady=5)
        self.aceax_description = ttk.Entry(self.glasses_scrollable_frame)
        self.aceax_description.pack(fill=tk.X, padx=10, pady=5)
        self.aceax_description.insert(0, "putsomething")

        self.aceax_options_frame = ttk.Frame(self.glasses_scrollable_frame)
    ...
    def generate_config(self):
        ...
        config += self.generate_cfg_glasses()
    ...
    def generate_cfg_glasses(self):
        cfg_glasses = "class CfgGlasses\n{\n"
        cfg_glasses += f"    class {self.glasses_base_class.get()};\n"
        cfg_glasses += f"    class {self.glasses_mother_class.get()} : {self.glasses_base_class.get()}\n    {{\n"
        cfg_glasses += f'        author = "{self.author.get()}";\n'
        cfg_glasses += "        scope = 0;\n"
        cfg_glasses += '        displayname = "This_IS_Hidden";\n'
        cfg_glasses += f'        model = "{self.model.get()}";\n'
        cfg_glasses += f'        picture = "{self.picture.get()}";\n'
        cfg_glasses += f'        icon = "{self.icon.get()}";\n'
        cfg_glasses += "        mass = 4;\n"
        cfg_glasses += "        hiddenSelections[] =\n        {\n"
        for selection in self.hidden_selections:
            cfg_glasses += f'            "{selection.get()}",\n'
        cfg_glasses = cfg_glasses.rstrip(',\n') + "\n"
        cfg_glasses += "        };\n"
        cfg_glasses += "        hiddenSelectionsTextures[] =\n        {\n"
        for texture in self.hidden_textures:
            cfg_glasses += f'            "{texture.get()}",\n'
        cfg_glasses = cfg_glasses.rstrip(',\n') + "\n"
        cfg_glasses += "        };\n"
        cfg_glasses += "        mode = 4;\n"
        cfg_glasses += "    };\n"

        for variant in self.generate_glasses_variants():
            cfg_glasses += self.generate_glasses_variant(variant)

        cfg_glasses += "};\n\n"
        return cfg_glasses

    def generate_glasses_variants(self):
        options = []
    ...
    def generate_glasses_variant(self, variant):
        variant_name = '_'.join(variant)
        glasses = f"    class {self.glasses_mother_class.get()}_{variant_name} : {self.glasses_mother_class.get()}\n    {{\n"
        glasses += "        scope = 2;\n"
        glasses += f'        displayName = "Glasses {variant_name}";\n'
        glasses += "        hiddenSelectionsTextures[] =\n        {\n"
    ...
    def generate_xtd_gear_models(self):
        ...
        xtd_gear_models += "    class CfgGlasses\n    {\n"
        xtd_gear_models += f"        class {self.aceax_model.get()}\n        {{\n"
        xtd_gear_models += "            label = \"Glasses\";\n"
    ...
    def generate_xtd_gear_info(self, variant):
        variant_name = '_'.join(variant)
        info = f"        class {self.glasses_mother_class.get()}_{variant_name}\n        {{\n"
...
if __name__ == "__main__":
    root = tk.Tk()
    app = GlassesConfigGenerator(root)
    root.mainloop()

...
class NVGConfigGenerator:
    def __init__(self, master):
        self.master = master
        self.master.title("Arma 3 NVG Config Generator")
        ...
        self.nvg_frame = ttk.Frame(self.notebook)

        self.notebook.add(self.cfg_patches_frame, text="CfgPatches")
        self.notebook.add(self.nvg_frame, text="NVG")

        self.create_cfg_patches_widgets()
        self.create_nvg_widgets()
    ...
    def create_nvg_widgets(self):
        self.nvg_canvas = tk.Canvas(self.nvg_frame)
        self.nvg_scrollbar = ttk.Scrollbar(self.nvg_frame, orient="vertical", command=self.nvg_canvas.yview)
        self.nvg_scrollable_frame = ttk.Frame(self.nvg_canvas)

        self.nvg_scrollable_frame.bind(
            "<Configure>",
            lambda e: self.nvg_canvas.configure(
                scrollregion=self.nvg_canvas.bbox("all")
            )
        )

        self.nvg_canvas.create_window((0, 0), window=self.nvg_scrollable_frame, anchor="nw")
        self.nvg_canvas.configure(yscrollcommand=self.nvg_scrollbar.set)

        self.nvg_canvas.pack(side="left", fill="both", expand=True)
        self.nvg_scrollbar.pack(side="right", fill="y")

        self.create_cfg_weapons_widgets()
        self.create_aceax_widgets()

    def create_cfg_weapons_widgets(self):
        ttk.Label(self.nvg_scrollable_frame, text="CfgWeapons").pack(anchor='w', padx=10, pady=5)
        
        ttk.Label(self.nvg_scrollable_frame, text="NVG Base Class:").pack(anchor='w', padx=10, pady=5)
        self.nvg_base_class = ttk.Entry(self.nvg_scrollable_frame)
        self.nvg_base_class.pack(fill=tk.X, padx=10, pady=5)
        self.nvg_base_class.insert(0, "NVGoggles")

        ttk.Label(self.nvg_scrollable_frame, text="NVG Mother Class:").pack(anchor='w', padx=10, pady=5)
        self.nvg_mother_class = ttk.Entry(self.nvg_scrollable_frame)
        self.nvg_mother_class.pack(fill=tk.X, padx=10, pady=5)
        self.nvg_mother_class.insert(0, "TFN_Bungiee")

        ttk.Label(self.nvg_scrollable_frame, text="Model:").pack(anchor='w', padx=10, pady=5)
        self.model = ttk.Entry(self.nvg_scrollable_frame)
        self.model.pack(fill=tk.X, padx=10, pady=5)
        self.model.insert(0, ".p3d")

        ttk.Label(self.nvg_scrollable_frame, text="Picture:").pack(anchor='w', padx=10, pady=5)
        self.picture = ttk.Entry(self.nvg_scrollable_frame)
        self.picture.pack(fill=tk.X, padx=10, pady=5)
        self.picture.insert(0, ".paa")

        ttk.Label(self.nvg_scrollable_frame, text="Icon:").pack(anchor='w', padx=10, pady=5)
        self.icon = ttk.Entry(self.nvg_scrollable_frame)
        self.icon.pack(fill=tk.X, padx=10, pady=5)
        self.icon.insert(0, ".paa")

        self.hidden_selections_frame = ttk.Frame(self.nvg_scrollable_frame)
        ...
        self.hidden_textures_frame = ttk.Frame(self.nvg_scrollable_frame)
    ...
    def create_aceax_widgets(self):
        ttk.Label(self.nvg_scrollable_frame, text="AceAx").pack(anchor='w', padx=10, pady=5)

        ttk.Label(self.nvg_scrollable_frame, text="AceAx Model:").pack(anchor='w', padx=10, pady=5)
        self.aceax_model = ttk.Entry(self.nvg_scrollable_frame)
        self.aceax_model.pack(fill=tk.X, padx=10, pady=5)
        self.aceax_model.insert(0, "NVGG")

        ttk.Label(self.nvg_scrollable_frame, text="AceAx Description:").pack(anchor='w', padx=10, pady=5)
        self.aceax_description = ttk.Entry(self.nvg_scrollable_frame)
        self.aceax_description.pack(fill=tk.X, padx=10, pady=5)
        self.aceax_description.insert(0, "NVG")

        self.aceax_options_frame = ttk.Frame(self.nvg_scrollable_frame)
    ...
    def generate_cfg_weapons(self):
        ...
        cfg_weapons += f"    class {self.nvg_base_class.get()};\n"
        cfg_weapons += f"    class {self.nvg_mother_class.get()} : {self.nvg_base_class.get()}\n    {{\n"
        cfg_weapons += f'        author = "{self.author.get()}";\n'
        cfg_weapons += "        scope = 0;\n"
        cfg_weapons += '        displayname = "This_IS_Hidden";\n'
        cfg_weapons += f'        model = "{self.model.get()}";\n'
        cfg_weapons += f'        picture = "{self.picture.get()}";\n'
        cfg_weapons += f'        icon = "{self.icon.get()}";\n'
        cfg_weapons += "        mass = 1;\n"
        cfg_weapons += "        hiddenSelections[] =\n        {\n"
        for selection in self.hidden_selections:
            cfg_weapons += f'            "{selection.get()}",\n'
        cfg_weapons = cfg_weapons.rstrip(',\n') + "\n"
        cfg_weapons += "        };\n"
        cfg_weapons += "        hiddenSelectionsTextures[] =\n        {\n"
        for texture in self.hidden_textures:
            cfg_weapons += f'            "{texture.get()}",\n'
        cfg_weapons = cfg_weapons.rstrip(',\n') + "\n"
        cfg_weapons += "        };\n"
        cfg_weapons += "        visionMode[] =\n        {\n"
        cfg_weapons += '            "Normal",\n'
        cfg_weapons += '            "NVG"\n'
        cfg_weapons += "        };\n"
        cfg_weapons += "        class ItemInfo\n        {\n"
        cfg_weapons += "            type = 616;\n"
        cfg_weapons += "            hmdType = 16;\n"
        cfg_weapons += f'            uniformModel = "{self.model.get()}";\n'
        cfg_weapons += "            hiddenSelections[] =\n            {\n"
        for selection in self.hidden_selections:
            cfg_weapons += f'                "{selection.get()}",\n'
        cfg_weapons = cfg_weapons.rstrip(',\n') + "\n"
        ...
        for variant in self.generate_nvg_variants():
            cfg_weapons += self.generate_nvg_variant(variant)

        cfg_weapons += "};\n\n"
        return cfg_weapons

    def generate_nvg_variants(self):
        options = []
    ...
    def generate_nvg_variant(self, variant):
        variant_name = '_'.join(variant)
        nvg = f"    class {self.nvg_mother_class.get()}_{variant_name} : {self.nvg_mother_class.get()}\n    {{\n"
        nvg += f'        displayName = "NVG {variant_name}";\n'
        nvg += "        scope = 2;\n"
        nvg += "        hiddenSelectionsTextures[] =\n        {\n"
    ...
    def generate_xtd_gear_models(self):
        ...
        xtd_gear_models += f'            options[] = {{{", ".join(options)}}};\n'
    ...
    def generate_xtd_gear_info(self, variant):
        variant_name = '_'.join(variant)
        info = f"        class {self.nvg_mother_class.get()}_{variant_name}\n        {{\n"
    ...
    def show_debug_window(self):
        ...
        debug_info += f"NVG Base Class: {self.nvg_base_class.get()}\n"
        debug_info += f"NVG Mother Class: {self.nvg_mother_class.get()}\n"
        debug_info += f"Model: {self.model.get()}\n"
        debug_info += f"Picture: {self.picture.get()}\n"
        debug_info += f"Icon: {self.icon.get()}\n"

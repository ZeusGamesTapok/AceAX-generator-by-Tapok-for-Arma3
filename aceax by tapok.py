...
class UniformConfigGenerator:
    ...
    def add_aceax_option(self):
        ...
        def remove_value(value_frame):
            index = list(values_frame.children.values()).index(value_frame)
            values.pop(index)
            icons.pop(index)
            textures.pop(index)
            change_ingame_vars.pop(index)
            action_labels.pop(index)
            value_frame.destroy()
    ...
    def generate_uniform_variants(self):
        ...
        for option in self.aceax_options:
            values = [value.get() for value in option['values'] if value.get()]  # Only include non-empty values
            if not values:
    ...
    def generate_xtd_gear_models(self):
        ...
        for option in self.aceax_options:
            ...
            for i, value in enumerate(option['values']):
                if value.get():  # Check if the value is not empty
